...
const DashboardPage: React.FC = () => {
  ...
  const { events: workspaceEvents, isLoading: eventsLoading } = useEvents();
  ...
  // Compute stats from API events
  const activeEventsCount = workspaceEvents.length;
  const totalSizeBytes = workspaceEvents.reduce((acc, event) => acc + event.total_size_bytes, 0);
  ...
  workspaceEvents.forEach(event => {
    event.collaborators?.forEach(collab => uniqueCollaborators.add(collab.id));
  });
