...
import { SHARED_EVENTS, addSimulatedEmail } from '../../constants';
...
const ClientAccessPage: React.FC = () => {
  ...
  const handlePinContinue = () => {
    ...
    // Validation: Email must match the customer email associated with the event
    if (email.toLowerCase().trim() !== event.customerEmail?.toLowerCase().trim()) {
    ...
    setTimeout(() => {
        ...
        addSimulatedEmail({
            ...
            body: `Your one-time password for accessing "${event.title}" is: ${newOtp}. This code will expire shortly.`
        });
  ...
  const handleOtpVerify = () => {
    ...
    setIsLoading(true);
    setTimeout(() => {
        if (enteredOtp === generatedOtp || enteredOtp === '000000') { // 000000 as dev bypass
            setIsLoading(false);
