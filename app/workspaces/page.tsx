...
const WorkspacesPage: React.FC = () => {
  ...
  const handleEditMember = (member: any) => {
    ...
    setInviteForm({
      ...
      role: member.role,
      selectedStudioIds: member.allowedWorkspaceIds || ['all'],
      message: ''
    });
  ...
  const handleSendInvite = () => {
    ...
    let accessScope: string | string[] = 'Full Access';
    if (!isAllStudios) {
        accessScope = inviteForm.selectedStudioIds
            .map(id => workspaces.find(ws => ws?.id === id)?.name || '')
            .filter(Boolean);
        
        if (accessScope.length === 1) {
            accessScope = accessScope[0];
        }
    }
  ...
  const getStudioDropdownLabel = () => {
    ...
    if (selectedStudioIds.length === 0) return 'NO ACCESS YET';
            ...
            {isAdmin && (
              <button 
                onClick={() => {
                  ...
                  else if (activeTab === 'TeamMembers') { setEditingMemberId(null); setInviteForm({ fullName: '', email: '', location: '', role: 'Studio Member', selectedStudioIds: ['all'], message: '' }); setIsAddMemberModalOpen(true); }
        ...
        {activeTab === 'TeamMembers' && isAdmin && (
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
             <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 md:mb-8 gap-4 text-start">
                ...
                <Button onClick={() => { setEditingMemberId(null); setInviteForm({ fullName: '', email: '', location: '', role: 'Studio Member', selectedStudioIds: ['all'], message: '' }); setIsAddMemberModalOpen(true); }} className="bg-[#0F172A] hover:bg-slate-800 text-white font-bold px-6 h-11 md:h-12 rounded-xl text-[10px] md:text-xs uppercase tracking-widest w-full sm:w-auto shadow-lg">
      ...
      <Modal isOpen={isAddMemberModalOpen} onClose={() => setIsAddMemberModalOpen(false)} title={editingMemberId ? "Edit Global Member" : "Invite Global Member"} className="max-w-4xl w-full" contentClassName="p-0 overflow-hidden">
        <div className="flex flex-col max-h-[85vh]">
          <div className="flex-1 p-6 sm:p-12 space-y-10 bg-white overflow-y-auto custom-scrollbar pb-24 text-start">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 sm:gap-10">
                  <div className="space-y-2 text-start">
                      <label className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] ml-0.5">FULL NAME *</label>
                      <div className="relative group"><div className="absolute start-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-slate-900 transition-colors"><User size={18} /></div><input type="text" placeholder="John Doe" value={inviteForm.fullName} onChange={e => setInviteForm({...inviteForm, fullName: e.target.value})} className="w-full ps-11 pe-4 py-3.5 bg-slate-50 border border-transparent rounded-xl text-sm font-semibold normal-case text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none text-start shadow-inner" /></div>
                  </div>
                  <div className="space-y-2 text-start">
                  ...
                  <div className="space-y-2 text-start">
                      ...
                      <div className="relative group">
                          ...
                          <Select 
                              ...
                              options={assignableRoles.map(r => ({ label: `${r.name.toUpperCase()} [${r.level === 'organization' ? 'ORG' : 'STUDIO'}]`, value: r.name }))}
                  ...
                  {!editingMemberId && (
                    <div className="space-y-2 text-start"><label className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.15em] ml-0.5">LOCATION</label><div className="relative group"><div className="absolute start-3.5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-slate-900 transition-colors"><MapPin size={18} /></div><input type="text" placeholder="City, Country" value={inviteForm.location} onChange={e => setInviteForm({...inviteForm, location: e.target.value})} className="w-full ps-11 pe-4 py-3.5 bg-slate-50 border border-transparent rounded-xl text-sm font-semibold normal-case text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none text-start shadow-inner" /></div></div>
                  )}
              </div>
          ...
          <div className="px-6 py-6 sm:px-12 border-t border-slate-100 bg-slate-50/50 flex flex-col sm:flex-row items-center justify-between gap-4 z-40">
              ...
              <Button onClick={handleSendInvite} disabled={!inviteForm.fullName.trim() || !inviteForm.email.trim() || !EMAIL_REGEX.test(inviteForm.email.trim())} className="w-full sm:w-auto sm:min-w-[280px] bg-[#0F172A] hover:bg-[#1E293B] text-white font-black h-14 rounded-xl shadow-2xl shadow-slate-900/10 text-[11px] uppercase tracking-[0.2em] active:scale-95 transition-all">{editingMemberId ? "SAVE MEMBER DETAILS" : "SEND PLATFORM INVITE"}</Button>
          </div>
        </div>
      </Modal>
