...
import { SHARED_EVENTS, saveGuestToRegistry, loadWorkspaces, addSimulatedEmail } from '../../constants';
...
const GuestAccessPage: React.FC = () => {
    ...
    setTimeout(() => {
        if (enteredPin === CORRECT_PIN) {
            ...
            addSimulatedEmail({
                ...
                subject: 'Your Guest Access Verification Code',
                body: `Hello ${formData.firstName},\n\nYour security verification code for accessing the event gallery is: ${newOtp}. This code will expire shortly.`
            });
            ...
            setError('Incorrect PIN. Try 9522.');
            setIsLoading(false);
            setPin(['', '', '', '']);
  ...
  const handleOtpVerify = () => {
    ...
    setTimeout(() => {
        if (enteredOtp === generatedOtp || enteredOtp === '000000') {
            // --- CAPTURE GUEST DATA ---
            const foundEvent = SHARED_EVENTS.find(e => e.id === eventId);
            const workspaces = loadWorkspaces();
            const foundWorkspace = workspaces.find(w => w.id === foundEvent?.workspaceId);
            
            const sessionId = `gst_${Date.now()}`;
            const guestRecord: GuestRecord = {
                id: sessionId,
                name: `${formData.firstName} ${formData.lastName}`.trim(),
                email: formData.email.trim(),
                phone: formData.phone.trim(),
                workspaceId: foundEvent?.workspaceId || 'unknown',
                workspaceName: foundWorkspace?.name || 'Unknown Studio',
                eventId: eventId || 'unknown',
                eventName: foundEvent?.title || 'Unknown Event',
            ...
            // Persist to lead registry
            saveGuestToRegistry(guestRecord);
            
            // Store session ID to track downloads later
            sessionStorage.setItem('photmo_guest_session_id', sessionId);
  ...
  const startCamera = async () => {
      try {
          ...
          setTimeout(() => {
            setScanStatus('Analyzing Biometrics...');
          }, 1500);
      ...
      return (
        <div className="min-h-screen bg-[#0F172A] flex flex-col items-center justify-center p-4 font-sans text-white relative overflow-hidden">
                 ...
                 {isScanning && (
                    <>
                        ...
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-64 border border-blue-500/30 rounded-[3rem] z-10">
                            ...
                            <div className="absolute top-4 left-4 w-4 h-4 border-t-2 border-l-2 border-blue-400 drop-shadow-[0_0_5px_rgba(59,130,246,0.8)]"></div>
                            <div className="absolute top-4 right-4 w-4 h-4 border-t-2 border-r-2 border-blue-400 drop-shadow-[0_0_5px_rgba(59,130,246,0.8)]"></div>
                            <div className="absolute bottom-4 left-4 w-4 h-4 border-b-2 border-l-2 border-blue-400 drop-shadow-[0_0_5px_rgba(59,130,246,0.8)]"></div>
                            <div className="absolute bottom-4 right-4 w-4 h-4 border-b-2 border-r-2 border-blue-400 drop-shadow-[0_0_5px_rgba(59,130,246,0.8)]"></div>
                            <div className="absolute top-1/2 left-1/2 w-2 h-2 bg-blue-500 rounded-full -translate-x-1/2 -translate-y-1/2 shadow-[0_0_10px_rgba(59,130,246,1)]"></div>
                        </div>
                    </>
                 )}
             </div>
             ...
             <div className="text-center z-10 space-y-6 w-full max-w-[320px]">
                 <p className={isScanning ? "text-sm font-bold text-blue-300 tracking-wider uppercase animate-[blink-text_1s_infinite]" : "text-sm font-medium text-slate-300"}>
                    {scanStatus}
                 </p>
                 ...
                 <Button
                    onClick={startCamera}
                    disabled={isScanning}
                    className={`w-full h-12 rounded-xl text-sm font-bold transition-all shadow-lg hover:shadow-blue-900/20 ${
                        isScanning 
                        ? 'bg-blue-600/80 text-white cursor-wait border-none' 
     ...
     <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-4 font-sans">
        <div className="bg-white w-full max-w-[480px] rounded-[2rem] shadow-2xl p-8 sm:p-10 text-center relative overflow-hidden animate-in fade-in zoom-in-95 duration-300 border border-slate-100">
                    ...
                    <div className="flex justify-center gap-4">
                        {pin.map((digit, idx) => (
                            <input
                                ...
                                className={`w-14 h-14 rounded-[1rem] border text-center text-2xl font-bold focus:outline-none focus:ring-4 focus:ring-offset-0 transition-all ${
                                    error 
            ...
            <button
                ...
                className={`w-full font-black py-4 rounded-2xl transition-all flex items-center justify-center shadow-xl uppercase tracking-[0.1em] text-sm active:scale-[0.98] ${
                    isFormValid 
                        ? 'bg-[#0F172A] hover:bg-slate-800 text-white shadow-slate-900/10' 
                        : 'bg-slate-100 text-slate-300 cursor-not-allowed shadow-none'
                }`}
