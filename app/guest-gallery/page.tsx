...
import { SHARED_EVENTS, SharedEvent, SharedMediaItem, incrementGuestDownloadCount } from '../../constants';
...
const GuestGalleryPage: React.FC = () => {
  ...
  const [foundPhotos, setFoundPhotos] = useState<SharedMediaItem[]>([]);
  const [isDownloading, setIsDownloading] = useState(false);
  const [selectedIds, setSelectedIds] = useState<Set<string>>(new Set());
  const [activeBranding, setActiveBranding] = useState<any>(null);
  
  // Mock User Data
  const GUEST_NAME = "Haider";
  ...
  useEffect(() => {
    if (eventId) {
      ...
      if (found) {
        ...
        // Only load photos if published
        if (found.status === 'Published') {
            const allItems = found.collections.flatMap(c => c.items);
            setFoundPhotos(allItems);
        } else {
            setFoundPhotos([]); // Clear photos if draft
        }
      }
    }
  }, [eventId]);
  ...
  return (
    <div className="min-h-screen bg-white font-sans flex flex-col">
        ...
        <footer className="py-8 text-center text-xs text-slate-400 border-t border-slate-50">
            <p>Powered by Enterprise Auth Portal AI Technology</p>
        </footer>
    </div>
  );
};
