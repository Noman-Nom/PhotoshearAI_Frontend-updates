...
const TeamPage: React.FC = () => {
  ...
  const [inviteForm, setInviteForm] = useState({ firstName: '', lastName: '', email: '', roleId: '', message: '' });
  ...
  const handleAddExistingToStudio = async (member: TeamMember) => {
    ...
    try {
      // Update member with role_id and workspaceIds (API mapper expects workspaceIds)
  ...
  const handleSendInvite = async () => {
    if (!inviteForm.firstName || !inviteForm.email || !inviteForm.roleId || !activeWorkspace?.id) return;
    try {
      await inviteMember({
      ...
      setIsAddModalOpen(false);
      setInviteForm({ firstName: '', lastName: '', email: '', roleId: '', message: '' });
    ...
    const handleRemoval = async (m: any, isPending: boolean) => {
      ...
      try {
        if (isPending) {
          ...
          // Use workspaceIds for API mapper 
                    ...
                    listToRender.map(member => {
                      ...
                      return (
                        <tr
                          key={member.id}
                          className={cn(
                            "hover:bg-slate-50/40 transition-colors group cursor-pointer list-item-transition",
                            isMutating && "optimistic-update card-loading"
                          )}
            ...
            {addMode === 'EXISTING' ? (
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                  <Input label="First Name" placeholder="John" value={inviteForm.firstName} onChange={e => setInviteForm({ ...inviteForm, firstName: e.target.value })} />
                  <Input label="Last Name" placeholder="Doe" value={inviteForm.lastName} onChange={e => setInviteForm({ ...inviteForm, lastName: e.target.value })} />
                </div>
                <Input label="Email Address" placeholder="john@example.com" value={inviteForm.email} onChange={e => setInviteForm({ ...inviteForm, email: e.target.value })} />
                <Select label="Studio Role" value={inviteForm.roleId} onChange={e => setInviteForm({ ...inviteForm, roleId: e.target.value })} options={[{ label: "SELECT A ROLE FROM LIST", value: "" }, ...studioLevelRoles.map(r => ({ label: r.name.toUpperCase(), value: r.id }))]} />
                <TextArea label="Personal Message (Optional)" placeholder="Welcome to the team!" value={inviteForm.message} onChange={e => setInviteForm({ ...inviteForm, message: e.target.value })} />
                <Button onClick={handleSendInvite} disabled={!inviteForm.firstName || !inviteForm.email || !EMAIL_REGEX.test(inviteForm.email.trim()) || !inviteForm.roleId} className="w-full bg-[#0F172A] h-14 text-xs font-black uppercase tracking-[0.2em] rounded-2xl shadow-xl">Send Studio Invitation</Button>
              </div>
            )}
          </div>
        </div>
