...
import { clientAccessApi, ClientGalleryItem, ClientGalleryResponse } from '../../services/clientAccessApi';
...
const ClientGalleryPage: React.FC = () => {
    ...
    useEffect(() => {
        const loadGallery = async () => {
            const token = sessionStorage.getItem('client_session_token');

            // If no token and not in simulation mode (localStorage), redirect to access page
            if (!token && !localStorage.getItem('fotoshare_events')) {
                // Check if we can find it in SHARED_EVENTS first (simulation fallback)
                const found = SHARED_EVENTS.find(e => e.id === eventId);
                if (found) {
                    setEvent(found);
                } else {
                    // No local event, no token -> redirect
                    navigate(`/client-access/${eventId}`);
                }
                return;
            }
            ...
            if (token) {
                try {
                    const gallery = await clientAccessApi.getGallery(token);

                    // Transform API response to SharedEvent format for compatibility
                    const mappedEvent: SharedEvent = {
                        id: gallery.event_id,
                        workspaceId: 'ws-1', // Placeholder
                        title: gallery.event_title,
                        date: new Date().toISOString(),
                        status: 'Published',
                        coverUrl: '',
                        totalPhotos: gallery.total_items,
                        totalVideos: 0,
                        totalSizeBytes: 0,
                        ...
                        branding: gallery.branding_enabled,
                        brandingId: gallery.branding_id
                    };
    ...
    const processBatchDownload = async (items: SharedMediaItem[]) => {
        ...
        try {
            // Sequential frontend downloads with branding/blob check
            for (const item of items) {
                // Process downloads sequentially to prevent browser throttling and UI freezing
                await downloadMediaWithBranding(item, activeBranding);
                // Small delay to ensure browser handles the download event cleanly
                await new Promise(r => setTimeout(r, 800));
            }
        } catch (error) {
            console.error("Batch download interrupted:", error);
        } finally {
            setIsDownloading(false);
        }
    };
    ...
    const handleDownloadAll = async () => {
        if (displayPhotos.length === 0) return;
        await processBatchDownload(displayPhotos);
    };

    const handleDownloadSelected = async () => {
        const itemsToDownload = displayPhotos.filter(p => selectedIds.has(p.id));
        if (itemsToDownload.length === 0) return;
        await processBatchDownload(itemsToDownload);
    };
                                ...
                                >
                                    {item.type === 'video' ? (
                                        <div className="w-full h-full flex items-center justify-center bg-slate-900 relative">
                                            ...
                                            {item.thumbnailUrl ? (
                                                <img src={item.thumbnailUrl} className="absolute inset-0 w-full h-full object-contain opacity-70 bg-slate-900" alt="" />
                                            ) : item.url ? (
                                                <video src={item.url} className="absolute inset-0 w-full h-full object-contain opacity-70 bg-slate-900" />
                                            ) : null}
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                            {selectedPerson?.items.map((item) => (
                                <div
                                    ...
                                    onClick={() => handlePreview(item)}
                                >
                                    <img src={item.thumbnailUrl || item.url} className="w-full h-full object-contain group-hover:scale-105 transition-transform duration-500 bg-slate-50" alt="" />
